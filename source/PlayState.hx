...
class PlayState extends MusicBeatState
{
	...
	public var dialogue2:Array<String> = CoolUtil.coolTextFile(Paths.txt('bringiton/bringitonDialogueEnd'));
	...
	//yes we are baquette
	var backFence:FlxSprite;
	var frontFence:FlxSprite;
	var fpDansers:FlxSprite;
	var effectDevantToutLeMonde:FlxSprite;
	...
	override public function create()
	{
		...
		switch (songLowercase)
		{
			...
			case 'thinkso':
				dialogue = CoolUtil.coolTextFile(Paths.txt('thinkso/thinksoDialogue'));
			case 'popyskweak':
				dialogue = CoolUtil.coolTextFile(Paths.txt('popyskweak/popyskweakDialogue'));
			case 'bringiton':
				dialogue = CoolUtil.coolTextFile(Paths.txt('bringiton/bringitonDialogue'));
				trace('bringiton not end 359');
		}
			...
			case 'kazual':
				{
					...
					var skyBG:FlxSprite = new FlxSprite(-120, -900).loadGraphic(Paths.image('kazsky'));
					skyBG.scrollFactor.set(0.2, 0.2);
					add(skyBG);
  
					var bg:FlxSprite = new FlxSprite(-480, -350).loadGraphic(Paths.image('kazbackstreet'));
					bg.antialiasing = true;
					bg.scrollFactor.set(0.9, 0.9);
					bg.active = false;
					add(bg);
  
					var frontStreet:FlxSprite = new FlxSprite(-580, -380).loadGraphic(Paths.image('kazstreet'));
					frontStreet.setGraphicSize(Std.int(frontStreet.width * 1.1));
					frontStreet.updateHitbox();
					frontStreet.antialiasing = true;
					frontStreet.scrollFactor.set(0.9, 0.9);
					frontStreet.active = false;
					add(frontStreet);
  
					var spDansers:FlxSprite = new FlxSprite(-435, -300);
					spDansers.frames = Paths.getSparrowAtlas('SP');
					spDansers.setGraphicSize(Std.int(spDansers.width * 1.4));
					spDansers.animation.addByPrefix('SP', "PG", 24);
					spDansers.animation.play('SP');
					spDansers.scrollFactor.set(0.9, 0.9);
					add(spDansers);
  
					fpDansers = new FlxSprite(-465, -330);
					fpDansers.frames = Paths.getSparrowAtlas('frontperso');
					fpDansers.setGraphicSize(Std.int(fpDansers.width * 1.4));
					fpDansers.animation.addByPrefix('FPanim', "fp", 24);
					fpDansers.animation.play('FPanim');
					fpDansers.scrollFactor.set(0.9, 0.9);
					// adding it later ... layeing is anoying (L712)
  
					backFence = new FlxSprite(-435, -200).loadGraphic(Paths.image('kazfence'));
					backFence.updateHitbox();
					backFence.setGraphicSize(Std.int(spDansers.width * 1.4));
					backFence.antialiasing = true;
					backFence.scrollFactor.set(0.9, 0.9);
					backFence.active = false;
					//add(backFence);
					// adding it later ... layeing is anoying (L712)
  
  
					frontFence = new FlxSprite(-435, -200).loadGraphic(Paths.image('kazfrontfence'));
					frontFence.updateHitbox();
					frontFence.setGraphicSize(Std.int(spDansers.width * 1.4));
					frontFence.antialiasing = true;
					frontFence.scrollFactor.set(0.9, 0.9);
					frontFence.active = false;
  
  
					effectDevantToutLeMonde = new FlxSprite (-700, -400).loadGraphic(Paths.image('kazeffect'));
					effectDevantToutLeMonde.updateHitbox();
					effectDevantToutLeMonde.antialiasing = true;
					effectDevantToutLeMonde.setGraphicSize(Std.int(effectDevantToutLeMonde.width * 1.3));
					effectDevantToutLeMonde.scrollFactor.set(0.9, 0.9);
					effectDevantToutLeMonde.active = false;
		...
		switch (curStage)
		{
			...
			case 'kazstage':
				...
				if (SONG.song.toLowerCase() == 'bringiton'){
					dad.x -= 123;
					dad.y -= 180;
					dad.setGraphicSize(Std.int(gf.width * 1.19));
					// Camera plus basse
				...
				gf.x += 70;
				gf.y += 20;
				gf.setGraphicSize(Std.int(gf.width * 0.85));
			case 'schoolEvil':
				if(FlxG.save.data.distractions){
	...
	function beforeEnding():Void{
		var doof2:DialogueBox = new DialogueBox(false, dialogue2);
		// doof.x += 70;
		// doof.y = FlxG.height * 0.5;
		doof2.scrollFactor.set();
		doof2.finishThing = endThatFuckingSong;
	...
	function endThatFuckingSong():Void{endSong();}
	...
	function startSong():Void
	{
		...
		FlxG.sound.music.onComplete = beforeEnding;
		vocals.play();
		...
		else
		{
			if (isStoryMode)
			{
				...
				else
				{
					...
					switch(StringTools.replace(songFormat," ", "-").toLowerCase())
					{
						case 'bringiton':
							LoadingState.loadAndSwitchState(new VideoState("assets/videos/cinetwo/cinetwo.webm", new PlayState()));
						default:
